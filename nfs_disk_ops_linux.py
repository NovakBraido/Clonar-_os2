...
def configurar_servidor_nfs():
    ...
    mount_target = "/srv"  # Ou outro diretório de sua escolha
...
def transferir_imagem(arquivo_gz, dir_remoto):
    ...
    # Certifique-se de que o diretório remoto esteja montado antes de copiar
    stdout, stderr = executar_comando(f"cp {arquivo_gz} {dir_remoto}") # Substitua por scp se necessário
    if stdout:
        print(stdout)
    if stderr:
...
def menu_operacoes_disco():
    while True:
        print("\nOperações de Disco:")
        # ... (opções do menu - igual ao código anterior)
